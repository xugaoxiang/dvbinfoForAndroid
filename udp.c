    ...
    switch(addr->sa_family)
    {
        ...
        case AF_INET:
            if (setsockopt(s, SOL_IP, MCAST_JOIN_GROUP, &greq, sizeof(greq)) == 0) {
                printf("===djstava setsockopt succ ... \n\n");
                return true;
            }
            break;
        default:
...
int udp_open(const char *interface, const char *ipaddress, int port)
{
    ...
    for (struct addrinfo *ptr = addr; ptr != NULL; ptr = ptr->ai_next )
    {
        ...
        imr.imr_multiaddr.s_addr = inet_addr(ipaddress);
	    imr.imr_interface.s_addr = INADDR_ANY;
        if (setsockopt(s_ctl, SOL_IP, IP_ADD_MEMBERSHIP, &imr, sizeof (imr)) < 0)
        ...
        {
            printf("===djstava is_multicast failed...\n\n");
            close(s_ctl);
            s_ctl = -1;
